...
if (calculate == TRUE) {
  ...
  saveRDS(hp_fit_baseline_lm_log, 'models/hp_fit_baseline_lm_log.rds')
  saveRDS(time_fit_duration_baseline_lm_log,
          'models/time_fit_duration_baseline_lm_log.rds')
}

hp_fit_baseline_lm_log <- readRDS('models/hp_fit_baseline_lm_log.rds')
...
results_baseline_lm_log <-
  as.data.frame(
    cbind(
      ...
      'Train Time (min)' = round(as.numeric(time_fit_duration_baseline_lm_log, units = 'mins'), 1),
...
all_results <- rbind(all_results, 'Baseline Lin. Reg. Log' = results_baseline_lm_log)
...
submission_baseline_lm_log <-
  cbind('id' = hp_test_id,
        'price' = 10^(hp_pred_baseline_lm_log_test * sd(hp_train_A_log$price) + mean(hp_train_A_log$price)))
...
submission_baseline_lm_log_train_B <-
  cbind('id' = hp_test_id,
        'price' = 10^(hp_pred_baseline_lm_log_train_B * sd(hp_train_A_log$price) + mean(hp_train_A_log$price)))
...
all_real_results <- rbind(all_real_results, 'Baseline Lin. Reg. Log' = real_results_baseline_lm_log)
