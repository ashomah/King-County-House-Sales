...
residuals_baseline_ranger_log_all_fact <-
  resid(hp_fit_baseline_ranger_log_all_fact)
hp_pred_baseline_ranger_log_all_fact <-
  predict(hp_fit_baseline_ranger_log_all_fact,
          hp_train_B_log_all_fact_proc)
comp_baseline_ranger_log_all_fact <-
  ...
  as.data.frame(
    cbind(
      ...
      'Train Time (min)' = round(
        as.numeric(time_fit_duration_baseline_ranger_log_all_fact, units = 'mins'),
        1
      ),
...
png(
  paste0(
    'plots/',
    plot_counter,
    '. baseline_ranger_log_all_fact_varImp.png'
  ),
  width = 1500,
  height = 1000
)
...
png(
  paste0(
    'plots/',
    plot_counter,
    '. baseline_ranger_log_all_fact_residuals.png'
  ),
  width = 1500,
  height = 1000
)
...
png(
  paste0(
    'plots/',
    plot_counter,
    '. baseline_ranger_log_all_fact_parameters.png'
  ),
  width = 1500,
  height = 1000
)
...
submission_baseline_ranger_log_all_fact <-
  cbind(#'id' = hp_test_id,
    'price' = 10 ^ (
      hp_pred_baseline_ranger_log_all_fact_test * sd(hp_train_A_log$price) + mean(hp_train_A_log$price)
    ))
write.csv(
  submission_baseline_ranger_log_all_fact,
...
submission_baseline_ranger_log_all_fact_train_B <-
  cbind(#'id' = hp_test_id,
    'price' = 10 ^ (
      hp_pred_baseline_ranger_log_all_fact_train_B * sd(hp_train_A_log$price) + mean(hp_train_A_log$price)
    ))
...
all_real_results <-
  rbind(all_real_results, 'Baseline Ranger Log All Fact' = real_results_baseline_ranger_log_all_fact)
