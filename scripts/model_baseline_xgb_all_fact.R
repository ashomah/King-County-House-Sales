...
if (calculate == TRUE) {
  ...
  stopCluster(cl)
  time_fit_duration_baseline_xgb_all_fact <- time_fit_end - time_fit_start
  saveRDS(hp_fit_baseline_xgb_all_fact, 'models/hp_fit_baseline_xgb_all_fact.rds')
  saveRDS(time_fit_duration_baseline_xgb_all_fact,
          'models/time_fit_duration_baseline_xgb_all_fact.rds')
}

hp_fit_baseline_xgb_all_fact <- readRDS('models/hp_fit_baseline_xgb_all_fact.rds')
time_fit_duration_baseline_xgb_all_fact <-
  readRDS('models/time_fit_duration_baseline_xgb_all_fact.rds')

residuals_baseline_xgb_all_fact <- resid(hp_fit_baseline_xgb_all_fact)
hp_pred_baseline_xgb_all_fact <-
  predict(hp_fit_baseline_xgb_all_fact, hp_train_B_proc_all_fact)
comp_baseline_xgb_all_fact <- data.frame(obs = hp_train_B_proc_all_fact$price,
...
results_baseline_xgb_all_fact <-
  as.data.frame(
    cbind(
      ...
      'Train Time (min)' = round(as.numeric(time_fit_duration_baseline_xgb_all_fact, units = 'mins'), 1),
...
png(
  paste0('plots/', plot_counter, '. baseline_xgb_all_fact_residuals.png'),
  width = 1500,
  height = 1000
)
...
png(
  paste0('plots/', plot_counter, '. baseline_xgb_all_fact_parameters.png'),
  width = 1500,
  height = 1000
)
...
write.csv(submission_baseline_xgb_all_fact,
...
all_real_results <- rbind(all_real_results, 'Baseline XGBoost All Fact' = real_results_baseline_xgb_all_fact)
